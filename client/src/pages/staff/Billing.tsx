...
function buildBillFromArrangement(
  arr: Arrangement,
  catalogItems: ServiceCatalogItem[],
  packages: (ServiceCatalogItem & { includedItems: ServiceCatalogItem[] })[]
): BillData {
  ...
  const merchandiseItems: BillItem[] = [];
  const cashAdvanceItems: BillItem[] = [];

  let packageName = "No Package Selected";
  const selectedPkg = packages.find((p) => p.id === sel.packageId);
  ...
  (sel.customItems || []).forEach((ci, idx) => {
    const item: BillItem = { id: `custom-${idx}`, description: ci.description, amount: ci.amount };
    if (ci.section === "service") serviceItems.push(item);
    else if (ci.section === "merchandise") merchandiseItems.push(item);
    else cashAdvanceItems.push(item);
  });
  ...
  return {
    ...
    sections: {
      ...
      cashAdvances: { title: "C. Cash Advances", items: cashAdvanceItems },
    },
    credits: [],
    taxRate: 0.0945,
  };
...
export default function Billing() {
  ...
  const { data: packages = [] } = useQuery<(ServiceCatalogItem & { includedItems: ServiceCatalogItem[] })[]>({
  ...
  const handleSend = (type: string) => {
    toast({ title: "Sent Successfully", description: `${type} has been sent to the family.` });
  };
      ...
      <div className="min-h-screen bg-background flex flex-col font-sans">
        ...
        <div className="bg-background border-b border-white/5 py-3">
          <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <span className="text-sm font-medium text-primary">{billData.packageName}</span>
          </div>
        </div>
        ...
        <main className="flex-grow container mx-auto px-4 md:px-6 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative">
            <div className="lg:col-span-2 space-y-8">
              {[billData.sections.services, billData.sections.merchandise, billData.sections.cashAdvances].map((section, sIdx) => {
                const sectionTotal = calculateSectionTotal(section);
                const sectionKey = sIdx === 0 ? "servicesTotal" : sIdx === 1 ? "merchandiseTotal" : "cashAdvancesTotal";
                if (section.items.length === 0) return null;
                return (
                  <section key={sIdx} className="bg-card rounded-lg border border-white/5 p-6 md:p-8">
                    ...
                    <div className="space-y-4">
                      {section.items.map((item) => (
                        <div key={item.id} className="flex justify-between items-start group">
                          ...
                          <span className={cn("font-mono text-sm md:text-base", item.isOverridden ? "text-amber-400" : "text-muted-foreground")}>
                            ${item.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            {item.isOverridden && <span className="text-xs ml-1">*</span>}
                          </span>
                        </div>
                      ))}
                    </div>
                    {sIdx === 1 && (
                      <div className="mt-4 pt-3 border-t border-white/5 text-xs text-muted-foreground">
                        Sales tax applies to merchandise items per Louisiana law.
                      </div>
                    )}
                  </section>
                );
              ...
              <div className="text-xs text-muted-foreground max-w-2xl mt-8 leading-relaxed">
                ...
                {billData.sections.services.items.some((i) => i.isOverridden) || billData.sections.merchandise.items.some((i) => i.isOverridden) ? (
                  <p className="mt-2 text-amber-400/70">* Indicates a price adjustment from the standard catalog price.</p>
                ) : null}
              </div>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-neutral-900 border border-primary/20 rounded-lg p-6 md:p-8 sticky top-32 shadow-2xl">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Sales Tax ({(billData.taxRate * 100).toFixed(2)}% on merchandise)</span>
